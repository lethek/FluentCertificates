...
public record CertificateBuilder
{
    ...
    /// <summary>Gets the elliptic curve used when generating an ECDsa key, or <see langword="null"/> to use the default curve (nistP256).</summary>
    public ECCurve? ECCurve { get; init; }
    ...
    /// <summary>
    /// Sets the elliptic curve to use when generating an ECDsa key.
    /// </summary>
    /// <remarks>
    /// <para>
    /// This applies only to keys generated by the builder itself. A key supplied through
    /// <see cref="SetKeyPair"/> already carries its own curve and takes precedence, in the same way a
    /// supplied key takes precedence over <see cref="KeyLength"/>.
    /// </para>
    /// <para>
    /// Setting a curve while <see cref="KeyAlgorithm"/> is anything other than
    /// <see cref="KeyAlgorithm.ECDsa"/> is rejected by <see cref="Validate"/> rather than ignored, since
    /// there is no key the caller could have meant by it.
    /// </para>
    /// </remarks>
    /// <param name="value">The named or explicit curve to generate the key on.</param>
    /// <returns>A new instance of <see cref="CertificateBuilder"/> with the specified curve.</returns>
    public CertificateBuilder SetECCurve(ECCurve value)
        => this with { ECCurve = value };
