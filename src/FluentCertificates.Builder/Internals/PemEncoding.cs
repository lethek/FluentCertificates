...
internal static class PemEncoding
{
    ...
    public static PemFields Find(ReadOnlySpan<char> pemData)
    {
        if (!TryFind(pemData, out PemFields fields)) {
            throw new ArgumentException("No PEM encoded data found.", nameof(pemData));
        }
    ...
    public static int GetEncodedSize(int labelLength, int dataLength)
    {
        ...
        if (labelLength < 0)
            throw new ArgumentOutOfRangeException(nameof(labelLength), "A positive number is required.");
        if (dataLength < 0)
            throw new ArgumentOutOfRangeException(nameof(dataLength), "A positive number is required.");
        if (labelLength > MaxLabelSize)
            throw new ArgumentOutOfRangeException(nameof(labelLength), "The encoded PEM size is too large to represent as a signed 32-bit integer.");
        if (dataLength > MaxDataLength)
            throw new ArgumentOutOfRangeException(nameof(dataLength), "The encoded PEM size is too large to represent as a signed 32-bit integer.");
        ...
        if (int.MaxValue - encodedDataLengthWithBreaks < totalEncapLength)
            throw new ArgumentException("The encoded PEM size is too large to represent as a signed 32-bit integer.");
    ...
    public static bool TryWrite(ReadOnlySpan<char> label, ReadOnlySpan<byte> data, Span<char> destination, out int charsWritten)
    {
        ...
        if (!IsValidLabel(label))
            throw new ArgumentException("The specified label is not valid.", nameof(label));
