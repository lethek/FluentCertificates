...
- **Breaking:** A certificate under an end-entity `CertificateUsage` with no `Issuer` is refused when a `SignatureGenerator` signs it with a key that is not the subject's own, which would name the certificate as its own issuer while another key vouched for it.
