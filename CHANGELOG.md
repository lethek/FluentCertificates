...
- **Breaking:** A certificate or signing request is refused when a basic constraints or key usage extension contradicts the `CertificateUsage` profile about whether the certificate is a certificate authority or may sign certificates, or carries a value that does not decode, or that carries further bytes after the value it encodes.
- **Breaking:** `CertificateBuilder.CreateCertificateSigningRequest` refuses a request signed by a key other than the one it certifies, since a PKCS#10 signature exists to prove the requester holds that key.
...
- **Breaking:** A certificate or signing request is refused when its `Issuer` publishes a Subject Key Identifier whose value does not decode, rather than naming that issuer by bytes taken from the value without decoding it.
...
- **Breaking:** A certificate with an empty `Subject` and no Subject Alternative Name is refused, as RFC 5280 s4.2.1.6 requires that extension of one. A signing request is not.
- **Breaking:** A certificate or signing request is refused when a Subject Alternative Name extension carries no entries, or does not decode.
...
[Unreleased]: https://github.com/lethek/FluentCertificates/compare/v0.22.0...HEAD
[0.22.0]: https://github.com/lethek/FluentCertificates/compare/v0.21.0...v0.22.0
